...
function tableExists(db: SqliteDatabase, table: string) { return Boolean(db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?").get(table)); }
...
export function migrateLegacyApplications(db: SqliteDatabase) {
  ...
  db.transaction(() => {
    const applications = db.prepare("SELECT * FROM applications").all() as Array<Record<string, string | null>>;
    const notes = tableExists(db, "application_notes") ? db.prepare("SELECT * FROM application_notes").all() as Array<Record<string, string | null>> : [];
    const changes = tableExists(db, "application_status_changes") ? db.prepare("SELECT * FROM application_status_changes").all() as Array<Record<string, string | null>> : [];
    const artifacts = tableExists(db, "application_artifacts") ? db.prepare("SELECT * FROM application_artifacts").all() as Array<Record<string, string | null>> : [];
    ...
    for (const app of applications) {
      const id = app.id!; const created = app.created_at!; const updated = app.updated_at!;
      ...
      const addTask = (title: string | null, due: string | null, at: string) => { if (!title || ["rejected", "archived"].includes(app.status || "")) return; const key = normalized(title) + "|" + (due ?? ""); if (taskPairs.has(key)) return; taskPairs.add(key); insertTask.run(randomUUID(), id, title.trim(), due, at, at); };
      ...
      for (const note of notes.filter((row) => row.application_id === id)) {
        ...
        else insertActivity.run(note.id || randomUUID(), id, "note", note.body, null, note.created_at, note.created_at);
      }
      for (const change of changes.filter((row) => row.application_id === id)) insertActivity.run(change.id || randomUUID(), id, "status_change", change.note || `Status changed to ${change.to_status}`, JSON.stringify({ fromStatus: change.from_status, toStatus: change.to_status }), change.created_at, change.created_at);
      for (const artifact of artifacts.filter((row) => row.application_id === id)) insertArtifact.run(artifact.id, id, artifact.type, artifact.title, artifact.file_path, artifact.content_type || "text/markdown", artifact.created_at, artifact.updated_at);
    }
