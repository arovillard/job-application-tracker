...
function opportunity(id: string) { const row = getDatabase().prepare(`${projection} WHERE o.id = ?`).get(id); return row ? mapOpportunity(row) : null; }
function mapActivity(row: any): OpportunityActivity { return { id: row.id, opportunityId: row.opportunity_id, type: row.type, body: row.body, metadata: row.metadata_json ? JSON.parse(row.metadata_json) : null, occurredAt: row.occurred_at, createdAt: row.created_at }; }
function mapTask(row: any): OpportunityTask { return { id: row.id, opportunityId: row.opportunity_id, title: row.title, dueDate: row.due_date, state: row.state, sourceActivityId: row.source_activity_id, completedAt: row.completed_at, createdAt: row.created_at, updatedAt: row.updated_at }; }
function mapArtifact(row: any): OpportunityArtifact { return { id: row.id, opportunityId: row.opportunity_id, type: row.type, title: row.title, filePath: row.file_path, contentType: row.content_type, createdAt: row.created_at, updatedAt: row.updated_at }; }
