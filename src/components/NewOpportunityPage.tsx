...
export function NewOpportunityPage() {
  ...
  return <main className="app-shell app-shell--narrow"><header className="app-header"><div><p className="app-header__eyebrow">New opportunity</p><h1 className="app-header__title">{type === "job" ? "Add a job" : "Add a connection"}</h1></div><Link className="button" href="/">Back to opportunities</Link></header>
    ...
    <section className="tracker-panel"><header className="tracker-panel__header"><h2 className="tracker-panel__title">{type === "job" ? "Job details" : "Connection details"}</h2></header>{type === "job" ? <JobOpportunityForm mode="create" isSubmitting={saving} onSubmit={(payload: JobCreationPayload) => create(payload)} /> : <ConnectionOpportunityForm mode="create" isSubmitting={saving} onSubmit={(payload: ConnectionCreationPayload) => create(payload)} />}</section>
  </main>;
}
