...
export function OpportunityTaskList({ tasks, pendingTaskId = null, onAction, onAddTask, today = new Date().toISOString().slice(0, 10) }: {
  ...
  return <div className="task-list"><section className="next-action-card"><header className="tracker-panel__header"><h2 className="tracker-panel__title">Next action</h2></header>{primary ? row(primary) : <button className="button button--primary" type="button" onClick={(event) => onAddTask?.(event.currentTarget)}>Set a next action</button>}</section><section className="tracker-panel"><header className="tracker-panel__header"><h2 className="tracker-panel__title">Tasks</h2><span className="tracker-panel__meta">{open.length}</span></header>{open.length ? open.map(row) : <p>No remaining open tasks.</p>}{history.length ? <details><summary>Task history ({history.length})</summary>{history.map(row)}</details> : null}</section></div>;
}
