...
function TaskRow({ task, pendingTaskId, onAction, today, primary = false }: { task: OpportunityTask; pendingTaskId: string | null; onAction: (task: OpportunityTask, action: TaskAction, dueDate?: string | null) => void | Promise<void>; today: string; primary?: boolean }) {
  const [dueDate, setDueDate] = useState(task.dueDate ?? "");
  const pending = pendingTaskId === task.id;
  return <div className={`task-item${primary ? " task-item--primary" : ""}`}><div className="task-item__content">{primary ? <span className="task-item__eyebrow">Up next</span> : null}<strong>{task.title}</strong><span className="task-item__due">{dueState(task, today)}</span></div><div className="task-item__actions">{task.state === "open" ? <><button className="task-item__action task-item__action--complete" disabled={pending} type="button" onClick={() => void onAction(task, "complete")}>Complete</button><button className="task-item__action" disabled={pending} type="button" onClick={() => void onAction(task, "cancel")}>Cancel</button><div className="task-item__reschedule"><label><span>Move due date</span><input aria-label={`Reschedule ${task.title}`} type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></label><button className="task-item__action" disabled={pending} type="button" onClick={() => void onAction(task, "reschedule", dueDate || null)}>Reschedule</button></div></> : <button className="task-item__action" disabled={pending} type="button" onClick={() => void onAction(task, "reopen")}>Reopen</button>}</div></div>;
}
...
export function OpportunityTaskList({ tasks, pendingTaskId = null, onAction, onAddTask, today = getLocalCalendarDate() }: {
  ...
  return <section className="next-action-card actions-card"><header className="tracker-panel__header"><div><p className="panel-heading__eyebrow">Momentum</p><h2 className="tracker-panel__title">Actions</h2></div><span className="tracker-panel__meta">{tasks.filter((task) => task.state === "open").length}</span></header><div className="actions-card__primary">{primary ? <TaskRow task={primary} primary pendingTaskId={pendingTaskId} onAction={onAction} today={today} /> : <div className="actions-card__empty"><p>No next action planned.</p><button className="button button--primary" type="button" onClick={(event) => onAddTask?.(event.currentTarget)}>Set a next action</button></div>}</div>{open.length ? <section className="actions-card__other"><header><h3>Other tasks</h3><span>{open.length}</span></header>{open.map(row)}</section> : null}{history.length ? <details className="actions-card__history"><summary>Completed and cancelled ({history.length})</summary>{history.map(row)}</details> : null}</section>;
}
