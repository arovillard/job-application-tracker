...
export function OpportunityDetailPage({ opportunityId }: { opportunityId: string }) {
  ...
  return <main className="app-shell"><OpportunityDetailContent detail={detail} pendingTaskId={pendingTaskId} onTaskAction={taskAction} onStatusChange={(status) => void mutate(`/api/opportunities/${opportunityId}/status`, "PATCH", { status }).catch((caught) => setError(caught.message))} onRecordInteraction={() => setShowInteraction(true)} onAddTask={() => setShowTask(true)} onEdit={() => setShowEdit(true)} onArchive={() => void archive()} onDelete={() => void remove()} onCreateJob={() => setShowLinkedJob(true)} />
    ...
    {showInteraction ? <TrackerPanel title="Record interaction" className="interaction-composer"><InteractionComposer activityType={activityType} body={body} occurredDate={occurredDate} taskTitle={taskTitle} taskDueDate={taskDueDate} onActivityTypeChange={setActivityType} onBodyChange={setBody} onOccurredDateChange={setOccurredDate} onTaskTitleChange={setTaskTitle} onTaskDueDateChange={setTaskDueDate} onSubmit={(event) => { event.preventDefault(); void mutate(`/api/opportunities/${opportunityId}/activities`, "POST", { type: activityType, body, occurredAt: occurredDate ? `${occurredDate}T12:00:00.000Z` : null, task: taskTitle ? { title: taskTitle, dueDate: taskDueDate || null } : null }).then(() => { setShowInteraction(false); setBody(""); setTaskTitle(""); setOccurredDate(""); }).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to save interaction")); }} onCancel={() => setShowInteraction(false)} /></TrackerPanel> : null}
    {showTask ? <TrackerPanel title="Add task"><TaskComposer taskTitle={taskTitle} taskDueDate={taskDueDate} onTaskTitleChange={setTaskTitle} onTaskDueDateChange={setTaskDueDate} onSubmit={(event) => { event.preventDefault(); void mutate(`/api/opportunities/${opportunityId}/tasks`, "POST", { title: taskTitle, dueDate: taskDueDate || null }).then(() => { setShowTask(false); setTaskTitle(""); setTaskDueDate(""); }).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to add task")); }} onCancel={() => setShowTask(false)} /></TrackerPanel> : null}
    {showEdit ? <TrackerPanel title="Edit details">{detail.type === "job" ? <JobOpportunityForm initialValue={detail} submitLabel="Save details" onSubmit={async (input) => { try { await mutate(`/api/opportunities/${opportunityId}`, "PUT", input); setShowEdit(false); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save details"); } }} /> : <ConnectionOpportunityForm initialValue={detail} submitLabel="Save details" onSubmit={async (payload) => { try { await mutate(`/api/opportunities/${opportunityId}`, "PUT", payload.opportunity); setShowEdit(false); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save details"); } }} />}<div className="application-form__actions"><button className="button" type="button" onClick={() => setShowEdit(false)}>Cancel</button></div></TrackerPanel> : null}
    {showLinkedJob && detail.type === "connection" ? <TrackerPanel title={`Create job from ${detail.label}`}><JobOpportunityForm initialValue={{ type: "job", label: "", organization: detail.organization, status: "wishlist", priority: detail.priority, summary: null, url: null, source: "Connection", location: null, contact: `${detail.label}${detail.roleContext ? ` - ${detail.roleContext}` : ""}`, appliedDate: null, originOpportunityId: detail.id }} originOpportunityId={detail.id} onSubmit={async (input) => { try { const created = await mutate(`/api/opportunities/${detail.id}/jobs`, "POST", input); router.push(`/opportunities/${created.id}`); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to create linked job"); } }} submitLabel="Create linked job" /><div className="application-form__actions"><button className="button" type="button" onClick={() => setShowLinkedJob(false)}>Cancel</button></div></TrackerPanel> : null}
  </main>;
}
