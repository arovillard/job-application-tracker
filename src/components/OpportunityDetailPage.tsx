...
export function OpportunityDetailPage({ opportunityId }: { opportunityId: string }) {
  const router = useRouter(); const [detail, setDetail] = useState<OpportunityDetail | null>(null); const [error, setError] = useState<string | null>(null); const [status, setStatus] = useState<string | null>(null); const [pendingTaskId, setPendingTaskId] = useState<string | null>(null);
  const [surface, setSurface] = useState<DetailSurface | null>(null);
  const [interaction, setInteraction] = useState({ activityType: "note" as typeof humanActivityTypes[number], body: "", occurredDate: "", taskTitle: "", taskDueDate: "" });
  ...
  const open = (next: DetailSurface) => { setError(null); setStatus(null); setSurface(next); };
  const close = () => {
    if (surface?.kind === "interaction") setInteraction({ activityType: "note", body: "", occurredDate: "", taskTitle: "", taskDueDate: "" });
    if (surface?.kind === "task") setTask({ title: "", dueDate: "" });
    setError(null);
    setSurface(null);
  };
  const succeed = (message: string) => { setError(null); setSurface(null); setStatus(message); };
  ...
  return <main className="app-shell"><OpportunityDetailContent detail={detail} pendingTaskId={pendingTaskId} onTaskAction={taskAction} onStatusChange={(status) => void mutate(`/api/opportunities/${opportunityId}/status`, "PATCH", { status }).then(() => setStatus("Stage updated")).catch((caught) => setError(caught.message))} onRecordInteraction={() => open({ kind: "interaction" })} onAddTask={() => open({ kind: "task" })} onEdit={() => open({ kind: "edit" })} onArchive={() => void archive()} onDelete={() => void remove()} onCreateJob={() => open({ kind: "linked-job" })} />
    {status ? <p role="status">{status}</p> : null}
    {surface?.kind === "interaction" ? <Modal title="Record interaction" onClose={close}><>{error ? <p role="alert">{error}</p> : null}<InteractionComposer {...interaction} onActivityTypeChange={(activityType) => setInteraction((current) => ({ ...current, activityType }))} onBodyChange={(body) => setInteraction((current) => ({ ...current, body }))} onOccurredDateChange={(occurredDate) => setInteraction((current) => ({ ...current, occurredDate }))} onTaskTitleChange={(taskTitle) => setInteraction((current) => ({ ...current, taskTitle }))} onTaskDueDateChange={(taskDueDate) => setInteraction((current) => ({ ...current, taskDueDate }))} onCancel={close} onSubmit={(event) => { event.preventDefault(); void mutate(`/api/opportunities/${opportunityId}/activities`, "POST", { type: interaction.activityType, body: interaction.body, occurredAt: interaction.occurredDate ? `${interaction.occurredDate}T12:00:00.000Z` : null, task: interaction.taskTitle ? { title: interaction.taskTitle, dueDate: interaction.taskDueDate || null } : null }).then(() => { setInteraction({ activityType: "note", body: "", occurredDate: "", taskTitle: "", taskDueDate: "" }); succeed("Interaction recorded"); }).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to save interaction")); }} /></></Modal> : null}
    {surface?.kind === "task" ? <Modal title="Add task" onClose={close}><>{error ? <p role="alert">{error}</p> : null}<TaskComposer taskTitle={task.title} taskDueDate={task.dueDate} onTaskTitleChange={(title) => setTask((current) => ({ ...current, title }))} onTaskDueDateChange={(dueDate) => setTask((current) => ({ ...current, dueDate }))} onCancel={close} onSubmit={(event) => { event.preventDefault(); void mutate(`/api/opportunities/${opportunityId}/tasks`, "POST", { title: task.title, dueDate: task.dueDate || null }).then(() => { setTask({ title: "", dueDate: "" }); succeed("Task added"); }).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to add task")); }} /></></Modal> : null}
    {surface?.kind === "edit" ? <Modal size="wide" title="Edit details" onClose={close}><>{error ? <p role="alert">{error}</p> : null}{detail.type === "job" ? <JobOpportunityForm initialValue={detail} mode="edit" submitLabel="Save details" onCancel={close} onSubmit={async (payload) => { try { await mutate(`/api/opportunities/${opportunityId}`, "PUT", payload.opportunity); succeed("Details saved"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save details"); } }} /> : <ConnectionOpportunityForm initialValue={detail} mode="edit" submitLabel="Save details" onCancel={close} onSubmit={async (payload) => { try { await mutate(`/api/opportunities/${opportunityId}`, "PUT", payload.opportunity); succeed("Details saved"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save details"); } }} />}</></Modal> : null}
    {surface?.kind === "linked-job" && detail.type === "connection" ? <Modal size="wide" title={`Create job from ${detail.label}`} onClose={close}><>{error ? <p role="alert">{error}</p> : null}<JobOpportunityForm initialValue={{ type: "job", label: "", organization: detail.organization, status: "wishlist", priority: detail.priority, summary: null, url: null, source: "Connection", location: null, contact: `${detail.label}${detail.roleContext ? ` - ${detail.roleContext}` : ""}`, appliedDate: null, originOpportunityId: detail.id }} originOpportunityId={detail.id} mode="linked" submitLabel="Create linked job" onCancel={close} onSubmit={async (payload) => { try { const created = await mutate(`/api/opportunities/${detail.id}/jobs`, "POST", payload.opportunity); succeed("Linked job created"); router.push(`/opportunities/${created.id}`); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to create linked job"); } }} /></></Modal> : null}
  </main>;
}
