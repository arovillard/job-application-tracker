...
export function OpportunityDetailPage({ opportunityId }: { opportunityId: string }) {
  ...
  const mounted = useRef(true); const detailGeneration = useRef(0); const surfaceGeneration = useRef(0); const submitting = useRef(false);
  useEffect(() => () => { mounted.current = false; detailGeneration.current += 1; surfaceGeneration.current += 1; }, []);
  ...
  const close = useCallback(() => { surfaceGeneration.current += 1; submitting.current = false; setIsSubmitting(false); setInteraction(emptyInteraction); setTask(emptyTask); setDialogError(null); setSurface(null); }, []);
  const open = useCallback((next: DetailSurface) => { surfaceGeneration.current += 1; submitting.current = false; setIsSubmitting(false); setDialogError(null); setStatus(null); setSurface(next); }, []);
  const applyDialogResult = useCallback((generation: number, next: OpportunityDetail, message: string, onSuccess?: (detail: OpportunityDetail) => void) => { if (mounted.current && generation === surfaceGeneration.current) { setDetail(next); setDialogError(null); setSurface(null); setStatus(message); submitting.current = false; setIsSubmitting(false); onSuccess?.(next); } }, []);
  const submitDialog = useCallback(async (request: () => Promise<OpportunityDetail>, message: string, fallback: string, onSuccess?: (detail: OpportunityDetail) => void) => { if (submitting.current) return; submitting.current = true; setIsSubmitting(true); const generation = surfaceGeneration.current; setDialogError(null); try { applyDialogResult(generation, await request(), message, onSuccess); } catch (caught) { if (mounted.current && generation === surfaceGeneration.current) { setDialogError(caught instanceof Error ? caught.message : fallback); submitting.current = false; setIsSubmitting(false); } } }, [applyDialogResult]);
  const taskAction = async (task: OpportunityTask, action: TaskAction, dueDate?: string | null) => { const generation = ++surfaceGeneration.current; setPendingTaskId(task.id); setPageError(null); try { const next = await mutate(`/api/opportunities/${opportunityId}/tasks/${task.id}`, "PATCH", { action, ...(action === "reschedule" ? { dueDate } : {}) }); if (mounted.current && generation === surfaceGeneration.current) setDetail(next); } catch (caught) { if (mounted.current && generation === surfaceGeneration.current) setPageError(caught instanceof Error ? caught.message : "Unable to update task"); } finally { if (mounted.current && generation === surfaceGeneration.current) setPendingTaskId(null); } };
  const archive = async () => { try { setDetail(await mutate(`/api/opportunities/${opportunityId}/status`, "PATCH", { status: "archived" })); } catch (caught) { setPageError(caught instanceof Error ? caught.message : "Unable to archive opportunity"); } };
  ...
  return <main className="app-shell"><OpportunityDetailContent detail={detail} pendingTaskId={pendingTaskId} onTaskAction={taskAction} onStatusChange={(status) => void mutate(`/api/opportunities/${opportunityId}/status`, "PATCH", { status }).then((next) => { setDetail(next); setStatus("Stage updated"); }).catch((caught) => setPageError(caught.message))} onRecordInteraction={() => open({ kind: "interaction" })} onAddTask={() => open({ kind: "task" })} onEdit={() => open({ kind: "edit" })} onArchive={() => void archive()} onDelete={() => void remove()} onCreateJob={() => open({ kind: "linked-job" })} />
