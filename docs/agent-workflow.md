...
JobTracker retrieves public posting content before invoking a provider. It manually revalidates every redirect, follows at most five redirects, uses a 15-second total deadline, accepts only HTML, XHTML, or plain text, and streams at most 2 MiB. HTML extraction uses Cheerio and removes scripts, styles, navigation, forms, and hidden content after separately reading `JobPosting` JSON-LD and explicit schema.org `JobPosting` microdata. Structured evidence is the first single posting object or scope with a nonempty title, hiring-organization name, and HTML-to-text description; fields are never combined across postings. JSON-LD supports string or array `@type`, bare or schema.org HTTP(S) URL forms, and nested `@graph` entries. Microdata values use `content` before visible text. Extracted, deduplicated plain text is limited to 32,000 Unicode characters.
...
Retrieval failure stops before provider invocation with `posting_retrieval_failed`. Preview approval then requires a complete structured `JobPosting` and deterministic field-scoped grounding: normalized preview company must appear as a whole phrase in structured company, role only in structured title, and an extractive summary needs at least 3 unique non-stopword terms of at least 3 characters with at least `max(3, ceil(60% of meaningful terms))` matches only in structured description. Full visible context remains prompt input but is never approval evidence. The gate has no title or occupation allowlist. Plain text, incomplete/bare declarations, and generic, login, error, or malformed pages fail safely even when unrelated visible text fully grounds the preview; unusual legitimate titles can pass when one complete structured posting agrees. The prompt requires posting-language responsibilities and forbids retrieval/access/login commentary. Unknown labels, unsupported public pages, hallucinated labels, and insufficiently grounded summaries stop with `preview_unusable` and **Try another URL** guidance. Both failures show only approved safe guidance and never expose approval. Only a usable preview reaches `awaiting_approval`.
