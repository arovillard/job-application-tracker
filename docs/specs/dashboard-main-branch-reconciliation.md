...
| Before | After | Why |
| --- | --- | --- |
| Current panel uses `Workspace / Opportunities` with an unstyled `<h1>` ([Dashboard.tsx:132](../../src/components/Dashboard.tsx#L132)). | Restore main's compact `Pipeline / Your opportunities` panel hierarchy with `<h2>`, retaining Opportunity Tracker branding. | Main's CSS explicitly styles the panel `<h2>`; the current `<h1>` falls back to browser margins and scale, weakening hierarchy. |
...
| Loading table renders literal text while skeleton CSS remains unused ([OpportunityTable.tsx:60](../../src/components/OpportunityTable.tsx#L60)). | Restore three stable skeleton rows with an accessible loading label. | Main avoids layout shift and communicates loading without replacing the entire table surface. |
