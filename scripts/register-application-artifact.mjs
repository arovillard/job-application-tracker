...
function main() { const input = args(process.argv.slice(2)); const automated = input["lock-token"] || input["expected-status"]; if (automated) { const allowed = new Set(["db", "opportunity-id", "type", "title", "file", "content-type", "lock-token", "expected-status"]); if (Object.keys(input).some((item) => !allowed.has(item))) throw new Error("Unknown automated registration option"); } const result = registerApplicationArtifact(input); process.stdout.write(`${JSON.stringify(result, null, 2)}\n`); }
