...
export function assertCurrentOpportunitySchema(db) {
  ...
  for (const [name, [table, expected]] of Object.entries(namedIndexes)) {
    ...
    if (!listed || !same(indexColumns(db, name), expected)) throw new Error(`index mismatch: ${name}`);
  }
  const uniqueArtifact = pragma(db, "PRAGMA index_list('opportunity_artifacts')").some((row) => row.unique === 1 && same(indexColumns(db, row.name), [["opportunity_id", 0], ["type", 0], ["file_path", 0]]));
